...
mongoose
  .connect(
    "mongodb+srv://sandy:mklop@cluster1.huyf1.mongodb.net/?retryWrites=true&w=majority&appName=Cluster1",
    {
...
app.post("/submit", upload.single("image"), async (req, res) => {
  try {
    const { name, category, type, priceRange, insurance, description } =
      req.body;
...
app.post("/cart", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error adding bike to cart", error: err.message });
  }
});
...
app.delete("/bikes/:id", async (req, res) => {
  try {
    ...
    const deletedBike = await FormData.findByIdAndDelete(id);
    if (!deletedBike) {
    ...
    res.status(500).send({ error: "Failed to delete bike." });
  }
});
